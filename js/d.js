...
// Objeto para almacenar los permisos por categoría
const permissionCategories = {
  roles: ['view_roles', 'create_roles'],
...
// Añadir la definición faltante de permissionsByRole
...
// Función para obtener nombre del usuario loggeado (faltante en el código original)
...
const listRoles = async () => {
  try {
    ...
    console.log("Obteniendo roles con token:", token.substring(0, 10) + "...");
...
const renderRolesTable = () => {
    ...
    roles.forEach(role => {
      ...
      row.innerHTML = `
        ...
        <td>
          <div class="action-buttons">
            <button onclick="selectRole('${roleName}')" class="icon-button view-button" title="Ver permisos">
      ...
      row.addEventListener("click", () => selectRole(roleName));
      tbody.appendChild(row);
    });
  };
  

// Seleccionar un rol para ver sus permisos
const selectRole = async (roleIdOrName) => {
  try {
    ...
    const titleElement = document.getElementById("selectedRoleName");
    if (titleElement) {
      const roleName = typeof role === 'string' ? role : role.name;
      titleElement.textContent = roleName.charAt(0).toUpperCase() + roleName.slice(1);
    }
    ...
    if (permissionsCategories) {
      permissionsCategories.style.display = "grid";
    }
    ...
    showError("Error al seleccionar rol: " + error.message);
  }
};

// Renderizar permisos del rol seleccionado
const renderPermissions = (role) => {
  // Obtener los permisos del rol
  let rolePermissions = [];
  
  if (typeof role === 'string') {
    // Es un nombre de rol predefinido
    rolePermissions = permissionsByRole[role] || [];
  ...
  Object.entries(permissionCategories).forEach(([category, permissions]) => {
    ...
    permissions.forEach(permission => {
      ...
      permissionItem.innerHTML = `
        <input type="checkbox" id="${permission}" ${hasPermission ? "checked" : ""} disabled>
